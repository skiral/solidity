...
bool SMTChecker::visit(IfStatement const& _node)
{
	...
	// If assignments happened in the meantime, the resulting assertions
	// are not present anymore becase of the "pop".
	// at the same time, we want to have the condition being present for
	// the assignment...
	// Actually the order of the assertions does not matter...

	// We currently cannot introduce new identifiers in blocks
...
bool SMTChecker::visit(WhileStatement const& )
{
//	_node.condition().accept(*this);
//	m_interface->push();
//	m_interface->addAssertion(expr(_node.condition()));
