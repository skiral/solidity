...
bool TypeChecker::visit(FunctionDefinition const& _function)
{
	...
	if (_function.isPayable())
	{
		...
		if (_function.isView())
			m_errorReporter.typeError(_function.location(), "Functions cannot be view/constant and payable at the same time.");
		if (_function.isPure())
			m_errorReporter.typeError(_function.location(), "Functions cannot be pure and payable at the same time.");
	}
